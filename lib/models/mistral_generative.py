...
@dataclass
class MistralGenerativeConfig:
    """
    Configuration class for Mistral Generative model.
...
class MistralGenerative(nn.Module):
    """
    PyTorch module for Mistral Generative model with PEFT (Parameter-efficient Fine-tuning).

    Attributes:
    - model_config (MistralGenerativeConfig): Configuration for the Mistral model.
    - data_config (DataSpec): Data specification for input data.
    """
    def __init__(self, model_config: MistralGenerativeConfig, data_config: DataSpec):
        super().__init__()

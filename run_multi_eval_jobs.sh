...
for epoch in $(seq 0 $epochs); do
    for member_id in $(seq 0 $members); do
        ./srun_single.sh ./run.sh  $current_dir/experiments/lora_ensembles/pipeline_lora_ens_eval.py --epoch $epoch --member_id $member_id --eval_dataset $eval_dataset --train_dataset $train_dataset
    done
done
